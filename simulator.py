...
Simulates N games between the Warriors and Cavs for a given CourtConfig.
Uses NN-weighted KDE shot location sampling and GB make/miss prediction.
A fatigue multiplier (logistic regression) applies a minor p_make decay
as a function of elapsed game minutes.
"""
...
class Simulator:
    ...
    def __init__(
        ...
        self.fatigue_model = fatigue_model  # optional FatigueModel instance
    ...
    def _simulate_shot(
        ...
        """
        Simulate a single shot outcome using the GB model + fatigue multiplier.
        ...
        # Apply fatigue multiplier (minor effect, max 5% reduction)
        if self.fatigue_model is not None:
            fatigue_mult = self.fatigue_model.get_multiplier(player_id, elapsed_minutes)
            p_make = float(np.clip(p_make * fatigue_mult, 0.0, 1.0))
    ...
    def _simulate_one_game(
        ...
        # Approximate minutes per shot attempt: 48 min / 104 FGA ≈ 0.462 min/shot
        # Used to advance the game clock as shots accumulate
        MINUTES_PER_SHOT = 48.0 / 104.0

        for players, team_id, opp_team_id in matchups:
            elapsed_minutes = 0.0  # reset clock for each team's half of simulation
            ...
            for pid in players:
                ...
                for _ in range(n_attempts):
                    ...
                    made, points = self._simulate_shot(
                        pid, zone, court_config, opp_team_id,
                    ...
                    # Advance game clock
                    elapsed_minutes = min(elapsed_minutes + MINUTES_PER_SHOT, 48.0)
