...
class HeatmapEngine:
    ...
    def render_heatmap(
        ...
        vmin, vmax = (0.0, 1.0) if is_probability else (None, None)
        ax.imshow(
            heatmap_array,
            origin="lower",
            extent=[-25, 25, 0, 47],
    ...
    def _draw_court_lines(
        ...
        Uses the standard court if court_config is None; otherwise draws the
        3PT arc for the given config.
        """
        arc_r = court_config.arc_radius if court_config else 23.75
        baseline_w = court_config.baseline_width if court_config else 50.0
        half_w = baseline_w / 2
        basket_to_baseline = court_config.basket_to_baseline if court_config else 5.25
        ...
        paint = mpatches.Rectangle((-8, -basket_to_baseline), 16, 19,
                                   fill=False, edgecolor=color, linewidth=lw)
        ax.add_patch(paint)

        # Free throw circle
        ft_circle = Arc((0, 19 - basket_to_baseline), 16, 16,
                        angle=0, theta1=0, theta2=180,
                        color=color, linewidth=lw)
        ax.add_patch(ft_circle)

        # 3PT arc
        # The arc spans from where it meets the sideline (or baseline) around the top
        # Compute angle where arc meets the sideline at x = ±half_w
        if arc_r >= half_w:
            y_at_sideline = np.sqrt(max(arc_r ** 2 - half_w ** 2, 0))
        ...
        if court_config is None or not court_config.corner3_eliminated:
            ...
            # Left corner segment: from baseline to where arc starts
            ax.plot([-half_w, -half_w], [-basket_to_baseline, y_arc_start],
                    color=color, linewidth=lw)
        ...
        # Baseline
        ax.plot([-half_w, half_w], [-basket_to_baseline, -basket_to_baseline],
                color=color, linewidth=lw)
