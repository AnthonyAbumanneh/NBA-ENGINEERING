...
class NeuralNetworkModel:
    ...
    def predict_batch(self, xs: np.ndarray, ys: np.ndarray, player_id) -> np.ndarray:
        ...
        probs = self.model.predict([coords, pid_arr], verbose=0, batch_size=len(xs)).ravel()
    ...
    def predict(self, x: float, y: float, player_id) -> float:
        ...
        prob = float(self.model.predict([coord, pid_arr], verbose=0)[0][0])
        return prob
    ...
    def predict_grid(self, player_id, x_step: float = 0.5, y_step: float = 0.5) -> np.ndarray:
        ...
        probs = self.model.predict([coords, pid_arr], verbose=0, batch_size=1024).ravel()
