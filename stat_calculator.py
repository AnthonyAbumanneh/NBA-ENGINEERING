...
class StatCalculator:
    ...
    def _filter_eligible_players(self) -> None:
        ...
        if "team_abbrev" in df.columns:
            ...
            warriors_abbrev = next(
                (a for a in abbrevs if "warrior" in str(a).lower()), None
            )
            cavs_abbrev = next(
                (a for a in abbrevs if "cavalier" in str(a).lower() or "cav" in str(a).lower()),
                None,
            )
    ...
    def _compute_possessions_per_minute(
        ...
        """
        # Target: ~88 FGA per game (real NBA playoff pace)
        TARGET_FGA_PER_GAME = 88.0
    ...
    def compute_defensive_ratings(self, secondary_df: pd.DataFrame) -> dict:
        ...
        # Build a name→team_id lookup using the known Warriors/Cavs team IDs
        # and their name keywords — more reliable than scanning team_abbrev column
        name_to_id: dict = {}
